...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="vfte003" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
...
export default {
  ...
  setup() {
    ...
    return { displayPageHeader, buildVersion, multiLanguages, labels, alreadyLoading };
  },
  mounted() {
    console.log("App: mounted ...");
    this.$nextTick(async () => {
      //ensure ui completed then invoke startApplication 
      startApplication("vfte003",(data) => {
        ...
        if(data.type=="language") {
          ...
          loadAndMergeLabel("vfte003", (success) => {
            if (success) {
  ...
  methods: {
    ...
    dataSelected(item,action) {
      ...
      if("edit"==action) {
        this.$refs.entryForm.retrieveRecord({product: item.product});
      } else if("delete"==action) {
        this.$refs.entryForm.startDeleteRecord({product: item.product});
      }
    },
    dataInsert(filters) {
      //listen action from search form
